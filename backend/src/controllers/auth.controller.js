...
import { emailVerificationMailGenContent, sendMail } from "../utils/mail";
...
export const register = async (req, res) => {
    ...
    try {
        ...
        return res.status(201).status(201).json(
            new ApiResponse(201, "User Created!", { user })
        )
    } catch (error) {
        console.log(`Error in register user! ${error}`);
        ApiError(500, `Error in register user!`)
    }
}
...
export const verifyUser = async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json(
            new ApiResponse(200, "User verified succesfully!")
        )
    } catch (error) {
        console.log(`Error in verify user! ${error}`);
        ApiError(500, `Error in verify user!`)
    }
}
...
export const loginUser = async (req, res) => {
    ...
    try {
        ...
        if (!user.isVerified) {
            throw new ApiError(400, "Please verify your account first to log in!")
        }

        const accessToken = jwt.sign({ id: user._id, role: user.role }, process.env.JWT_SECRET_ACCESS_TOKEN, { expiresIn: '30d' });
        ...
        ApiError(500, `Error while logging in user`)
    }
...
export const resendEmailVerification = async (req, res) => {
    ...
    try {
        ...
        ApiError(500, `Error while re-sending email verification!`)
    }
}
...
export const forgotPasswordRequest = async (req, res) => {
    ...
    try {
        ...
        await sendMail({ email, subject, mailGenContent: forgotPasswordMailGenContent(username, resetPasswordUrl) })

        return res.status(200).json(
            new ApiResponse(200, "Password reset link is sent to the email!")
        )
    } catch (error) {
...
export const changeCurrentPassword = async (req, res) => {
    ...
    try {
        ...
        ApiError(500, `Error while changing current password!`)
    }
}
...
export const getCurrentUser = async (req, res) => {
    ...
    try {
        const user = await User.findById(id).select("-password -forgotPasswordToken -forgotPasswordExpires");
        ...
        ApiError(500, `Error while fetching current user!`)
    }
}